...
SAMPLE_FILE="sample_annotation.txt"  # Tab-separated file with sample info
...
while IFS=$'\t' read -r SAMPLE_ID FILENAME; do
    ...
    if [ -f "${CLEAN_R1}" ] && [ -f "${CLEAN_R2}" ]; then
        ...
        bsub -J "spades_${SAMPLE_ID}" \
             -w "done(fastp_${SAMPLE_ID})" \
             -o "logs/spades_${SAMPLE_ID}.out" \
             -e "logs/spades_${SAMPLE_ID}.err" \
             -n 8 -R "rusage[mem=16000]" \
             "spades.py --careful -t 8 -m 16 \
                      -1 ${CLEAN_R1} \
                      -2 ${CLEAN_R2} \
                      -o ${OUT_DIR}"
