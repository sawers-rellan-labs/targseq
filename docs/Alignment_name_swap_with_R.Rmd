...
metadata <- read.csv("~/Desktop/seqid_label_1.csv")
...
width(original_alignment)[1]  # Sequence lengths
```
...
plot_output <- function(file, tree,data){ 
  ...
  dotTree(tree ,X,colors= pal,labels=TRUE, fsize=0.7,border="transparent",cex.dot=2)
  dev.off()
}

rownames(name_swap) <- name_swap$label_2
# check_names <- rownames(name_swap)
# names(check_names) <- name_swap$label_3
# check_names
# hpc1_UPGMA$tip.label
...
write.tree(out_tree, file="~/Desktop/hpc1_UPGMA.tre")
```
