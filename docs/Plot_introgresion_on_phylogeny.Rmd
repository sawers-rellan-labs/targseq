...
cat("extdata/\n")
cat("├── ancestry_mismatch.csv            # Input: Ancestry call mismatches with tree position per sample (if any)\n")
cat("├── donor_data.csv                   # Input: Environmental data from donors\n")
...
cat("                                     # Includes ancestry information (ancestry_call)\n")
...
# Get data from package installation
...
trimmed_ids <- gsub(" .*", "", seq_ids, perl=TRUE)
...
Now we'll create a comprehensive visualization of our phylogenetic tree with variant and ancestry information using ggtree, plus add environmental data as a heatmap.
...
Finally, let's add a sequence alignment view to complete our visualization:
...
Often we want to focus our analysis on a specific subset of taxa. In this case, we'll create a subset that includes only taxa with donor ancestry plus the B73 reference genome.
...
Now that we have our subset defined, let's extract the corresponding sequences and build a new tree:
...
Let's visualize the donor subset tree with the same approach we used for the full tree:

```{r donor_visualization}
...
The donor subset analysis allows us to focus specifically on the relationships between B73 and the donor accessions, making it easier to identify patterns of introgression or shared ancestry. By highlighting B73 in gold, we can quickly identify it as our reference genome in the visualization.

This subset approach is particularly useful when working with large datasets where the full tree might be too crowded to interpret easily. It also allows us to examine how specific groups of taxa (in this case, donor accessions) relate to a reference genome of interest.
...
cat("├── target_sequencing_tree_alignment.png # Output: Full tree with visualization\n")
cat("└── donor_subset_tree_alignment.png      # Output: Donor subset visualization\n")
```
...
- Traversing the tree structure to determine necessary rotations
...
- `ggtree()`: Creates the basic tree structure
- `geom_tippoint()`: Adds points at the tips representing data values
- `geom_tiplab()`: Adds labels at the tips
- `gheatmap()`: Adds heatmaps alongside the tree
...
4. Sequence alignment views that provide direct visualization of the underlying data
